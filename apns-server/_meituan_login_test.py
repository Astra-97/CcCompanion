...
from meituan_login import MEITUAN_LOGIN_ORIGIN, MeituanLoginError, MeituanLoginManager
...
class MeituanLoginManagerTests(unittest.TestCase):
    def setUp(self) -> None:
        self.now = 100.0
        self.runner = FakeRunner()
        self.manager = MeituanLoginManager(
    ...
    def start(self, contact: str = "kimi"):
        return self.manager.start(
            contact_id=contact, device_id=DEVICE, origin=MEITUAN_LOGIN_ORIGIN
        )
    ...
    def test_only_allowed_contact_and_origin_can_start(self):
        for contact, origin in (("mallory", MEITUAN_LOGIN_ORIGIN), ("kimi", "web")):
            with self.assertRaises(MeituanLoginError):
                self.manager.start(contact_id=contact, device_id=DEVICE, origin=origin)

    def test_needs_login_tracks_meituan_status(self):
    ...
    def test_http_body_limits_cover_meituan_endpoints(self):
        ...
        self.assertIn('"/meituan-login/start": 4 * 1024', post)
        self.assertIn('"/meituan-login/import": 24 * 1024', post)
